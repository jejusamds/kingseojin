...
function delContent(idx){
   if(confirm('해당팝업을 삭제하시겠습니까?')){
      document.location = "page_save.php?mode=mpdelete&idx=" + idx;
   }
}
//-->
</script>
	...
	.pagination {margin:0 auto;}
			...
			<div class="pageWrap">
				<div class="page-heading">
					...
					<ul class="breadcrumb">
						<li>팝업 설정</li>
						<li class="active">팝업관리 - MOBILE</li>
					</ul>
				</div>
...
$sql  = "";
$sql .= "	Select	COUNT(*) ";
$sql .= "	From	df_site_content_mobile ";
$sql .= "	Where	1 = 1 ";
$sql .= $addSql;
$total = $db->single($sql);
...
?>
						  <tr>
							...
							<td><?=$row[$i]['isuse']?></td>
							<td>
								<button class="btn btn-success btn-xs" type="button" onClick="location.href='mobile_popup_input.php?mode=mpupdate&idx=<?=$row[$i]['idx']?>&page=<?=$page?>&<?=$param?>';">수정</button>
				...
				<div class="box comMTop20 comMBottom20" style="width:978px;">
					<div class="comPTop20 comPBottom20">
						...
						<div class="comFRight comARight" style="width:10%; padding-right:10px;">
							<button class="btn btn-primary btn-sm" type="button" onClick="location.href='mobile_popup_input.php?page=<?=$page?>&<?=$param?>';">팝업등록</button>
						</div>
						<div class="clear"></div>
					</div>
				</div>
