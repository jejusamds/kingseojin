			...
			editor._.simpleuploadsFormInitFn = fnInitPicker = CKEDITOR.tools.addFunction( function() {
				window.setTimeout(function() {
					...
					picker.onchange=function() {
						...
						if (evdata.requiresImage && !isImageExtension(filePickerEditor, evdata.name))
						{
...
CKEDITOR.plugins.add( "simpleuploads",
{
	...
	lang : ['en','de','es','pl','ko'], // "en" the first one to use it as default
	...
	{
		if (typeof editor.config.simpleuploads_imageExtensions == "undefined")
			editor.config.simpleuploads_imageExtensions = "jpe?g|gif|png";
			...
			html = html.replace( /<img(.*?) src="data:image\/.{3,4};base64,.*?"(.*?)>/g, function( img )
				{
					...
					req += rn + "Content-Disposition: form-data; name=\"upload\"; filename=\"" + uploadData.name + "\"";
					req	+= rn + "Content-type: image/" + type;
					req += rn + rn + window.atob( imgData.split(',')[1] );
					req += rn + "--" + BOUNDARY;

					// Add extra fields if provided
					if (uploadData.extraFields)
					{
						var obj = uploadData.extraFields;
						for (var prop in obj)
						{
...
function isImageExtension(editor, filename)
{
...
{
	var isImage = isImageExtension( editor, data.name ),
...
function createXHRupload(editor, forceLink, data)
{
	var xhr = new XMLHttpRequest(),
		isImage = isImageExtension( editor, data.name ),
		attribute,
		target = xhr.upload,
		forImage;
...
function createSVGAnimation( file, id, editor )
{
		...
		if (element.filebrowser.action == "QuickUpload")
		{
			...
			if (!editor.config.simpleuploads_respectDialogUploads)
			{
				...
				element.onClick = function( evt )
				{
					...
					PickAndUploadFile(editor, imageDialog, evt, callback);
					return false;
				};
	...
	function pasteListener(e) {
		...
		var data = e.data.$.clipboardData || (editor.config.forcePasteAsPlainText && window.clipboardData);
		if (!data)
			return;
